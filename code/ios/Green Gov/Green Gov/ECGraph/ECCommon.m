...
+ (NSString *)sBetweenW1:(NSString *)w1 W2:(NSString *)w2 OfS:(NSString *)aString;
{
	...
	{
		unsigned long location1 = range1.location ;
		unsigned long location2 = range2.location ;
...
{
	unsigned long stringLength = [aString length];
...
+(NSString *)sSplitByThousandOfS:(NSString *)aString
{
	...
	int thousandCount = 0;
	for(unsigned long i = [aString length] - 1;i >= startIndex;--i)
	{
