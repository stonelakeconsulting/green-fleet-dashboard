...
{
    [color set];
    UILabel *lb_xaxisTitle;
    UILabel *lb_yaxisTitle;
    UILabel *lb_graphicTitle;
    
    lb_xaxisTitle = [ECCAST(UIView,_delegate) viewWithTag:1000];
    if (lb_xaxisTitle==nil)
    {
        lb_xaxisTitle = [[UILabel alloc] initWithFrame:CGRectMake(_xaxisStart.x , _xaxisStart.y + 20 , _xaxisLength, 18)];
        ...
        lb_xaxisTitle.tag = 1000;
        [ECCAST(UIView,_delegate) addSubview:lb_xaxisTitle];
        [lb_xaxisTitle release];
    }
    lb_xaxisTitle.text = _xaxisTitle;
	
    lb_yaxisTitle = [ECCAST(UIView,_delegate) viewWithTag:1001];
    if (lb_yaxisTitle==nil)
    {
        lb_yaxisTitle = [[UILabel alloc] initWithFrame:CGRectMake(_xaxisStart.x - _yaxisLength/2 - 30, _yaxisEnd.y + _yaxisLength/2 , _yaxisLength, 18)];
        lb_yaxisTitle.tag = 1001;
    ...
    lb_graphicTitle = [ECCAST(UIView,_delegate) viewWithTag:1002];
    if (lb_graphicTitle==nil)
    {
        lb_graphicTitle = [[UILabel alloc] initWithFrame:CGRectMake(_xaxisStart.x , _yaxisEnd.y - 20  , _xaxisLength, 18)];
...
-(id)initWithFrame:(CGRect)frame withContext:(CGContextRef)context isPortrait:(BOOL)isPortrait
{
	if (self = [super init]) 
	{
		...
		_ySpacing = YSPACEING;
		//get XY-axis length
		_xaxisLength = fabs(_xaxisStart.x - _xaxisEnd.x);
		_yaxisLength = fabs(_yaxisStart.y - _yaxisEnd.y);
...
-(void)drawHistogramWithItems:(NSArray *)items lineWidth:(int)lineWidth color:(UIColor *)color
{
	...
	if (item.isPercentage)
    {
		...
		for (int i = 1; i <= YSTEPS_COUNT; ++i) 
		{
            ...
            {
                [self drawWords:[ECCommon sOfI:yName] AtPoint:CGPointMake(_xaxisStart.x-35,_xaxisStart.y - _ySpacingScale*i*_ySpacingLength - 8) color:[UIColor blackColor]];
                yName += (_maxY/YSTEPS_COUNT);
            }
            [self drawYLineAtPoint:CGPointMake(_xaxisStart.x,_xaxisStart.y - _ySpacingScale*i*_ySpacingLength)
						 lineWidth:2 color:[UIColor blackColor] highlighted:NO];
...
-(void)drawHistogramWithItem:(ECGraphItem *)item index:(int)index color:(UIColor *)color
{
    ...
    NSString *percentage = [NSString stringWithFormat:@"%ld",item.actualYValue];
    
    NSDictionary *textAttributes = @{NSFontAttributeName: [UIFont boldSystemFontOfSize:10.0]};
    ...
    if (item.actualName.length>0)
    {
        [color set];
        UILabel *itemTitle = [[UILabel alloc] initWithFrame:CGRectMake(10, _xaxisStart.y + 35 + (index*14) , 300, 18)];
        itemTitle.text = [NSString stringWithFormat:@"%@ = %@", item.name, item.actualName];
        itemTitle.textAlignment = NSTextAlignmentLeft;
        itemTitle.backgroundColor = [UIColor clearColor];
        itemTitle.font = [UIFont boldSystemFontOfSize:13];
        itemTitle.adjustsFontSizeToFitWidth = YES;
        itemTitle.tag = 1100+index;
        [ECCAST(UIView,_delegate) addSubview:itemTitle];
        [itemTitle release];
    }
}


+(void)removeTitles:(UIView*)view
{
    UILabel* lblTemp;
    int tag;
    
    /*
    lblTemp = [view viewWithTag:1000];
    lblTemp.text = @"";
    [lblTemp removeFromSuperview];
    lblTemp = [view viewWithTag:1001];
    lblTemp.text = @"";
    [lblTemp removeFromSuperview];
    lblTemp = [view viewWithTag:1002];
    lblTemp.text = @"";
    [lblTemp removeFromSuperview];
    */
    for (tag=1100; ; tag++)
    {
        lblTemp = [view viewWithTag:tag];
        if (lblTemp!=nil)
            [lblTemp removeFromSuperview];
        else
            break;
